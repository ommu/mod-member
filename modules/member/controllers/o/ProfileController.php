...
class ProfileController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = MemberProfile::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('MemberProfile', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'Member Profiles');
	...
	public function actionAdd() 
	{
		...
		if(isset($_POST['MemberProfile'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'id' => 'partial-member-profile',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'MemberProfile success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'MemberProfile success created.'));
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['MemberProfile'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-member-profile',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'MemberProfile success updated.').'</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'MemberProfile success updated.'));
					//$this->redirect(array('view','id'=>$model->profile_id));
					$this->redirect(array('manage'));
				}
			}
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				MemberProfile::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				MemberProfile::model()->deleteAll($criteria);
			}
		}
	...
	public function actionDelete($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				if($model->delete()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('manage'),
						'id' => 'partial-member-profile',
						'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'MemberProfile success deleted.').'</strong></div>',
					));
				}
			}
			...
			$this->pageTitle = Yii::t('phrase', 'MemberProfile Delete.');
			$this->pageDescription = '';
			$this->pageMeta = '';
