...
class LevelController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = MemberLevels::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('MemberLevels', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'Member Levels');
	...
	{
		$model=new MemberLevels;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['MemberLevels'])) {
			$model->attributes=$_POST['MemberLevels'];
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'MemberLevels success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'MemberLevels success created.'));
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['MemberLevels'])) {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'MemberLevels success updated.'));
					//$this->redirect(array('view','id'=>$model->level_id));
					$this->redirect(array('manage'));
				}
			}
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				MemberLevels::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				MemberLevels::model()->deleteAll($criteria);
			}
		}
	...
	public function actionDelete($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				if($model->delete()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('manage'),
						'id' => 'partial-member-levels',
						'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'MemberLevels success deleted.').'</strong></div>',
					));
				}
			}
			...
			$this->pageTitle = Yii::t('phrase', 'MemberLevels Delete.');
			$this->pageDescription = '';
			$this->pageMeta = '';
	...
	{
		$model = MemberLevels::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='member-levels-form') {
